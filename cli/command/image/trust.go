...
func trustedPull(ctx context.Context, cli command.Cli, imgRefAndAuth trust.ImageRefAndAuth, platform string, pullSource bool) error {
	...
	for i, r := range refs {
		...
		if err := imagePullPrivileged(ctx, cli, updatedImgRefAndAuth, false, platform, pullSource); err != nil {
			return err
		}
...
func imagePullPrivileged(ctx context.Context, cli command.Cli, imgRefAndAuth trust.ImageRefAndAuth, all bool, platform string, pullSource bool) error {
